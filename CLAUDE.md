...
- **`.tilde-tile` is for brand-coloured panels only** (currently the guild panel): two very large marks on a 640px repeat at 3.5% cream, sitting under the accent mark as texture. A denser tile across the whole page was tried and was far too busy, so the light sections keep the neutral dot grid and the dark feedback panel its engineering grid. Both stay quiet behind text in a way a logo never does.
- Favicons are the only brand files under `public/`, at the root, wired up in `layout.tsx`.
